...
class Node2VecGensim(Node2VecBase):
    ...
    def __init__(
        ...
        if window_size < 5 or vector_size > 40:
    ...
    def fit(self) -> GensimW2V:
        ...
        self.model = gensim.models.Word2Vec(
            sentences=all_walks, **self.w2v_params,
        )
        return self.model

    def get_vector(self, vertex_id: Optional[int] = None) -> Any:
        """
        Return vector associated with a node identified by the original node name/id
        """
        if vertex_id is None:
            return self.model.wv
        return self.model.wv.__getitem__(vertex_id)

    def save(self, file_path: str, file_name: str) -> None:
        """
        Save as embeddings in gensim.models.KeyedVectors format
        """
        self.model.save(file_path + "/" + file_name + ".model")
    ...
    def save_vectors(self, file_path: str, file_name: str) -> None:
        ...
        Save as embeddings in gensim.models.KeyedVectors format
        """
        self.model.wv.save_word2vec_format(file_path + "/" + file_name)
...
class Node2VecSpark(Node2VecBase):
    ...
    def get_vector(self, vertex_name: Optional[int] = None) -> DataFrame:
        """
        ...
        return self.model.getVectors().filter(f"word = {vertex_name}")
